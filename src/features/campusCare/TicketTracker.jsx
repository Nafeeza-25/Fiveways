...
export default function TicketTracker({ ticket, onAdvance }) {
  ...
  return (
    <motion.section initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} className="overflow-hidden rounded-[30px] border border-emerald-100 bg-white shadow-[0_18px_70px_rgba(6,78,59,.08)]">
      <div className="bg-gradient-to-br from-emerald-950 via-teal-900 to-slate-950 p-6 text-white sm:p-7"><p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.2em] text-emerald-300"><TicketCheck className="h-4 w-4" /> Demo ticket created</p><div className="mt-3 text-4xl font-black tracking-[-.055em]">{ticket.id}</div><div className="mt-3 text-xs leading-5 text-white/[.55]"><span><strong className="text-white/80">{ticket.category}</strong> · {ticket.location}</span>{ticket.description ? <span className="mt-1 block">{ticket.description}</span> : null}</div></div>
