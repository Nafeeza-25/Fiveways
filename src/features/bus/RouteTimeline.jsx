    ...
    <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[30px] border border-slate-200 bg-white p-6 shadow-soft-xl sm:p-7">
      ...
      <div className="relative mt-10 px-2 pb-4">
        ...
        <div className="relative grid gap-3 pt-14" style={{ gridTemplateColumns: `repeat(${bus.stops.length}, minmax(0,1fr))` }}>
          {bus.stops.map((stop) => <div key={stop} className={`min-w-0 text-center ${stop === bus.selectedStop ? 'route-stop--selected text-blue-700' : 'text-slate-400'}`}><div className={`mx-auto mb-2 grid h-7 w-7 place-items-center rounded-full border ${stop === bus.selectedStop ? 'border-blue-300 bg-blue-50' : 'border-slate-200 bg-white'}`}>{stop === bus.selectedStop ? <MapPin className="h-3.5 w-3.5" /> : <span className="h-1.5 w-1.5 rounded-full bg-slate-300" />}</div>{stop === bus.selectedStop && <small className="block text-[9px] font-black uppercase tracking-wider">Your stop</small>}<span className="block truncate text-[10px] font-bold sm:text-xs">{stop}</span></div>)}
        </div>
      </div>
